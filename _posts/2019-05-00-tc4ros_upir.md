...
<!-- TODO add some screenshot of the final result (and add it as a post thumbnail) -->
...
<!-- TODO add screenshot of new_connection events -->
...
2. `subscriber_link_message_write` or `subscriber_callback_start`, when a message is successfully removed from the queue
...
<!-- TODO add queues view screenshot -->
...
Finally, we can put everything together! From the view showing the queues over time, the user can select an individual message. The analysis then uses the above analyses to reconstruct and display the message's path accross queues, callbacks, and the network!

## Results/example

To illustrate this, I wrote a simple "pipeline" test case. A first node periodically publishes on a topic. A second node processes those messages, and re-publishes them on another topic. A third node does the same, and a fourth and last node prints the message's contents.

<!-- TODO add nodes pipeline testcase rqt_graph screenshot -->
